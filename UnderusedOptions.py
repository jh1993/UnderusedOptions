...
class ChimeraFamiliarSpellConduit(Spell):
    ...
    def on_init(self):
        ...
        self.range = 0
        self.description = "Casts %i of the wizard's %s%s%s%s or chaos sorcery spells with the highest max charges that can be cast from this tile, consuming a charge from them and counting as the wizard casting them." % (self.casts, "fire" if self.fire else "", ", " if self.fire and self.lightning else "", "lightning" if self.lightning else "", "," if self.fire and self.lightning else "")
    ...
    def cast_instant(self, x, y):
        ...
        for spell in self.get_wizard_spells():
            ...
            self.caster.level.event_manager.raise_event(EventOnSpellCast(spell_copy, self.caster.source.caster, target.x, target.y), self.caster.source.caster)
            casts_left -= 1
            if not casts_left:
                return
...
def modify_class(cls):
    ...
    if cls is TouchOfDeath:
        ...
        def on_init(self):
            ...
            self.upgrades['raise']= (1, 6, 'Touch of the Reaper', 'When a [living] target dies to touch of death, it is raise as a friendly Reaper for 6 turns.\nThe Reaper can cast Touch of Death with the same upgrades and bonuses as your own.')
        ...
        def cast_instant(self, x, y):
            ...
            if unit and not unit.is_alive() and Tags.Living in unit.tags and self.get_stat("raise"):
                reaper = Reaper()
    ...
    if cls is ChimeraFarmiliar:
        ...
        def get_description(self):
            return ("Summon a Chimera Familiar, which has [{minion_health}_HP:minion_health], and [{minion_resists}:damage] resistance to [fire], [lightning], and [physical].\n"
                    "Each turn, the chimera will copy [{casts}:num_targets] of your [fire], [lightning], or [chaos] [sorcery] spells that can be cast from its tile, preferring spells with the highest [max_charges:max_charges], consuming 1 charge from the spells copied. This counts as you casting those spells.\n"
