...
This mod requires my Bugs and Scams mod: https://github.com/jh1993/BugsAndScams
...
- Noxious Fumes buffed to 2 poison damage in 3 radius, and now benefits from radius bonuses.
- Corrosion upgrade replaced with Bioaccumulate (3 SP): Each turn, the target is inflicted with poison for a duration equal to this spell's damage stat, stacking with any existing poison the target has.
...
- The Magnetized Shards upgrade now makes it so that if there are no enemies in the area, no more shards will be fired. If it's smart enough to seek enemies, it should be smart enough to not hit allies. Not compatible with the new Particle Surge upgrade below.
...
Conductance:
- Instead of copying lightning spells cast at the target, the debuff will cause any lightning damage dealt to the target, before resistances, to also be dealt to a target in line of sight up to 4 tiles away. Targets with Conductance are prioritized, and if a target does not have Conductance, apply Conductance with duration equal to half of current remaining duration to it after dealing damage. Each target can only be damaged by Conductance once per turn.
...
- Feeding Frenzy (2 SP): On each activation, each allied unit in line of sight of the target that has bloodlust has a chance to gain another stack of bloodlust. The chance is equal to the percentage of the target's missing HP.
